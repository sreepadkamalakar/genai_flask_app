...
class Connections(WMLResource):
    ...
    async def aget_details(self, connection_id: str | None = None) -> dict:
        ...
        **Example:**
        ...
        .. code-block:: python
            ...
            connection_details = await client.connections.aget_details(connection_id)
        ...
        if connection_id:
            ...
            @self.async_httpx_retry()
            async def _aget_connection_by_id(self):
                return await self._client.async_httpx_client.get(
                    self._client._href_definitions.get_connection_by_id_href(
                        connection_id
                    ),
                    params=self._client._params(),
            ...
            response = await _aget_connection_by_id(self)
            ...
            @self.async_httpx_retry()
            async def _apost_connection(self):
            ...
            response = await _apost_connection(self)
    ...
    async def _aget_datasource_type_details(
        ...
        @self.async_httpx_retry()
        async def _aget_connection_data_type(self):
            return await self._client.async_httpx_client.get(
                self._client._href_definitions.get_connection_data_type_href(
                    datasource_type
                ),
        ...
        response = await _aget_connection_data_type(self)
    ...
    async def aget_datasource_type_details_by_name(
        ...
        **Example:**
        ...
        .. code-block:: python
            ...
            await client.connections.aget_datasource_type_details_by_name(datasource_type_name)
    ...
    async def aget_datasource_type_id_by_name(self, name: str) -> str:
        ...
        **Example:**
        ...
        .. code-block:: python
            ...
            await client.connections.aget_datasource_type_id_by_name('cloudobjectstorage')
    ...
    @staticmethod
    def async_httpx_retry(
        retries: int = 3,
        backoff_factor: float = 0.3,
        status_forcelist: tuple[int, ...] = (500, 502, 503, 504, 520, 521, 524),
    ):
        def decorator(function):
            @wraps(function)
            async def wrapper(self, *args, **kwargs) -> httpx.Response | None:
                ...
                for attempt in range(retries + 1):
                    ...
                    if (
                        response.status_code in status_forcelist
                    ) and attempt != retries:
                        await asyncio.sleep(
                            min(backoff_factor * pow(2.0, attempt), retries)
                        )
                    else:
                        break
                return response
