...
class Training(WMLResource):
    ...
    async def aget_details(
        ...
        **Examples**
        ...
        .. code-block:: python
            ...
            training_runs_details = await client.training.aget_details(limit=100, get_all=True)
            training_runs_details = []
            for entry in await client.training.aget_details(limit=100, asynchronous=True, get_all=True):
                training_runs_details.extend(entry)
    ...
    async def arun(
        ...
        **Examples**
        ...
        .. code-block:: python
            ...
            aggregator = await client.training.arun(aggregator_metadata, asynchronous=True)
            aggregator_id = client.training.get_id(aggregator)
        """
        # For CP4D, check if either spce or project ID is set
        self._client._check_if_either_is_set()
    ...
    async def acancel(
        self, training_id: str, hard_delete: bool = False, **kwargs
    ) -> Literal["SUCCESS"]:
    ...
    def _print_cos_object(
        self, run_id: str, cloud_file_name: str, on_start: Callable = lambda: {}
    ) -> None:
        on_start()
        run_details = self.get_details(run_id, _internal=True)
        if (
            "connection" not in run_details["entity"]["results_reference"]
            or run_details["entity"]["results_reference"].get("connection") is None
        ):
            print(" There is no connection details for this Training run.")
            return

        results_reference = run_details["entity"]["results_reference"]

        if results_reference["type"] == "connection_asset":
            connection_details = self._client.connections.get_details(
                results_reference["connection"]["id"]
            )
            endpoint_url = connection_details["entity"]["properties"]["url"]
            aws_access_key = connection_details["entity"]["properties"]["access_key"]
            aws_secret = connection_details["entity"]["properties"]["secret_key"]
        else:
            endpoint_url = results_reference["connection"]["endpoint_url"]
            aws_access_key = results_reference["connection"]["access_key_id"]
            aws_secret = results_reference["connection"]["secret_access_key"]
        ...
        try:
            if self._client.CLOUD_PLATFORM_SPACES:
                logs = get_from_json(
                    run_details, ["entity", "results_reference", "location", "logs"]
                )
                if logs is None:
                    print(" There is no logs details for this Training run.")
                    return

                key = logs + "/learner-1/" + cloud_file_name
                ...
                try:
                    key = (
                        "data/"
                        + (
                            run_details["metadata"].get("id")
                            or run_details["metadata"].get("guid")
                        )
                        + "/pipeline-model.json"
                    )
            ...
            if ex.response["Error"]["Code"] == "NoSuchKey":
                print(f"ERROR - Cannot find {cloud_file_name} in the bucket")
            else:
                print(ex)
                print("ERROR - Cannot get the training run log in the bucket")

    async def _aprint_cos_object(
        self, run_id: str, cloud_file_name: str, on_start: Callable = lambda: {}
    ) -> None:
        on_start()
        run_details = await self.aget_details(run_id, _internal=True)
        if (
            "connection" not in run_details["entity"]["results_reference"]
            or run_details["entity"]["results_reference"].get("connection") is None
        ):
        ...
        if results_reference["type"] == "connection_asset":
            connection_details = await self._client.connections.aget_details(
                results_reference["connection"]["id"]
            )
            endpoint_url = connection_details["entity"]["properties"]["url"]
            aws_access_key = connection_details["entity"]["properties"]["access_key"]
        ...
        try:
            if self._client.CLOUD_PLATFORM_SPACES:
                ...
                try:
                    ...
                    if ex.response["Error"]["Code"] == "NoSuchKey":
                        print(
                            " Error - There is no training logs found for the given training run id"
                        )
                        return
                    else:
                        print(ex)
    ...
    def _COS_logs(self, run_id: str, on_start: Callable = lambda: {}) -> None:
        self._print_cos_object(run_id, "training-log.txt", on_start)

    async def _aCOS_logs(self, run_id: str, on_start: Callable = lambda: {}) -> None:
        await self._aprint_cos_object(run_id, "training-log.txt", on_start)

    def _COS_metrics(self, run_id: str, on_start: Callable = lambda: {}) -> None:
        self._print_cos_object(run_id, "evaluation-metrics.txt", on_start)

    async def _aCOS_metrics(self, run_id: str, on_start: Callable = lambda: {}) -> None:
        await self._aprint_cos_object(run_id, "evaluation-metrics.txt", on_start)
    ...
    async def amonitor_logs(self, training_id: str) -> None:
        ...
        **Example:**
        ...
        await self._asimple_monitor_logs(
            training_id,  # type: ignore
            lambda: print_text_header_h1(
                "Log monitor started for training run: " + str(training_id)
            ),
    ...
    async def _asimple_monitor_logs(
        self, training_id: str, on_start: Callable = lambda: {}
    ) -> None:
        try:
        ...
        if (
            ...
            try:
                websocket.add_header(
                    bytes("Authorization", "utf-8"),
                    bytes(
                        "Bearer " + await self._client._auth_method.aget_token(),
                        "utf-8",
                    ),
                )
            except Exception:
                websocket.add_header(
                    bytes("Authorization", "utf-8"),
                    bytes("bearer " + await self._client._auth_method.aget_token()),
                )
    ...
    async def aget_metrics(self, training_id: str) -> ListType[dict]:
        ...
        **Example:**
        ...
        """
        Training._validate_type(training_id, "training_id", str, True)
        status = await self.aget_status(training_id)
        if "metrics" in status:
            return status["metrics"]
        else:
            details = await self.aget_details(training_id, _internal=True)
            if "metrics" in details:
                return details["metrics"]
            else:
                raise WMLClientError(
